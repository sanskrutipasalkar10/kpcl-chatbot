...
def get_dataframes():
    ...
    if global_kb_df is None:
        try:
            ...
            if os.path.exists(settings.KB_DATA_PATH):
                ...
                logger.warning(f"⚠️ Knowledge Base file not found: {settings.KB_DATA_PATH}")
                global_kb_df = pd.DataFrame()
        except Exception as e:
    ...
    if global_cost_df is None:
        try:
            ...
            if os.path.exists(settings.COST_DATA_PATH):
                ...
                logger.warning(f"⚠️ Cost Analysis file not found: {settings.COST_DATA_PATH}")
                global_cost_df = pd.DataFrame()
        except Exception as e:
