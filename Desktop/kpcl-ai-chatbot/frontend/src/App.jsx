...
function App() {
  ...
  const sendMessage = async (overrideText = null) => {
    ...
    try {
      ...
      // 1. Get URL from Vercel Env Var or fallback to local
      const rawBaseUrl = import.meta.env.VITE_API_URL || 'http://127.0.0.1:8000';
      
      // 2. Remove trailing slash to prevent double-slash 404 errors
      ...
      console.error("Connection Error:", error); // Helpful for F12 debugging
      setMessages(prev => [...prev, { 
        role: 'bot', 
        text: 'Connection to backend failed. Please ensure your Python server is running and the Vercel VITE_API_URL is correct.', 
        time: getCurrentTime() 
      }]);
    } finally {
      setIsLoading(false);
